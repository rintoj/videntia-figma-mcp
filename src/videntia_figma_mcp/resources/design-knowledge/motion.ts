...
- **Spring and custom curves on transitions:** \`add_prototype_link\` takes \`transitionEasing\` (including \`GENTLE\`/\`QUICK\`/\`BOUNCY\`/\`SLOW\`), plus \`easingFunctionCubicBezier\` for \`CUSTOM_CUBIC_BEZIER\` and \`easingFunctionSpring\` for \`CUSTOM_SPRING\`. It also takes \`direction\` (required for \`MOVE_IN\`/\`MOVE_OUT\`/\`PUSH\`/\`SLIDE_IN\`/\`SLIDE_OUT\`) and \`matchLayers\`. Use \`set_reactions\` for non-navigation actions (\`URL\`, \`BACK\`, \`CLOSE\`, \`SET_VARIABLE\`). **One action per reaction** — Figma's plugin API hangs on a reaction with several actions, Figma does accept several single-action reactions on the same trigger (verify in prototype playback that they fire as intended), or build the multi-action interaction in Figma's UI.
