...
export const DEFAULT_CHART_COLORS: Record<string, RGBAColor> = {
  ...
  "chart-7": { r: 0.98, g: 0.588, b: 0.118, a: 1.0 },
  ...
  const schema: ThemeSchema = {
    ...
    categories: {
      ...
      feedback: {
        ...
        count: 5,
        variables: ["overlay", "tooltip", "tooltip-foreground", "placeholder", "placeholder-foreground"],
      },
      utility: {
        name: "Utility Colors",
        count: 7,
        variables: ["muted", "muted-foreground", "selected", "selected-foreground", "border", "input", "ring"],
      },
      scales: {
        name: "Color Scales",
        count: 70,
...
export function isScaleVariable(variableName: string): boolean {
  return /^(primary|secondary|accent|success|info|warning|destructive)-(50|100|200|300|400|500|600|700|800|900)$/.test(
    variableName,
  );
}
