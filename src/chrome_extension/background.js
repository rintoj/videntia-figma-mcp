...
async function resizeOrEmulate(tab, frameWidth, frameHeight, opts = {}) {
  ...
  if (needsEmulation) {
    await attachDebuggerAndEmulate(tab.id, frameWidth, frameHeight, opts.deviceScaleFactor);
  } else {
    await detachDebuggerForTab(tab.id);
  }
...
async function attachDebuggerAndEmulate(tabId, width, height, deviceScaleFactor = 2) {
  const target = { tabId };
  const store = await chrome.storage.session.get(ATTACHED_TABS_KEY);
  const attached = store[ATTACHED_TABS_KEY] || {};
  if (!attached[tabId]) {
