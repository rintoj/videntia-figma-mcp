...
function updateLayerUI() {
  figmaLayerName.textContent = state.frameName ? `Figma · ${state.frameName}` : "Figma · —";
...
async function refreshBrowserDims() {
  ...
  try {
    ...
    state._availW = result.availW;
    state._availH = result.availH;
    state._availLeft = result.availLeft;
    state._availTop = result.availTop;
    updateFrameUI();
  } catch {}
}
...
async function matchDesign() {
  ...
  // Clamp the outer window to the visible screen work area — Chrome rejects
  // bounds that fall >50% off-screen ("Invalid value for bounds").
  const availW = state._availW || Infinity;
  const availH = state._availH || Infinity;
  const availLeft = state._availLeft || 0;
  const availTop = state._availTop || 0;
  const width = Math.floor(Math.min(state.frameWidth + chromeW, availW));
  const height = Math.floor(Math.min(state.frameHeight + chromeH, availH));
  try {
    await chrome.windows.update(tab.windowId, {
      width,
      height,
      // Pin position inside the work area so the resized window stays on-screen.
      left: availLeft,
      top: availTop,
      state: "normal",
    });
