...
export async function setEffects(params: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    const validEffects = (effects as Array<Record<string, unknown>>).map((effect) => {
      if (!effect["type"]) {
...
export async function setEffectStyleId(params: Record<string, unknown>): Promise<Record<string, unknown>> {
  const nodeId = params["nodeId"] as string | undefined;
  ...
  try {
    ...
    const nodePromise = (async () => {
      ...
      if (!foundStyle || foundStyle.type !== "EFFECT") {
        ...
        foundStyle =
          effectStyles.find(function (s) {
            return s.name === effectStyleId;
          }) || null;
        if (!foundStyle) {
      ...
      if (!foundStyle) {
        throw new Error(`Effect style not found: "${effectStyleId}". Pass a style ID or name (e.g. "shadow/md").`);
      }
...
export async function updateEffectStyle(params: Record<string, unknown>): Promise<Record<string, unknown>> {
...
export async function deleteEffectStyle(params: Record<string, unknown>): Promise<Record<string, unknown>> {
  const styleId = params["styleId"] as string | undefined;
