      ...
      <div class="flex items-center gap-2 px-3 py-1.5 shrink-0 bg-muted border-b border-border">
        ...
        >
          ...
          >
            ...
            {sel.showFilterPopup && <FilterPopup filterMode={sel.filterMode} onSelect={sel.handleFilterSelect} />}
          </div>
          <input
            type="text"
      ...
      <div class={"flex flex-col flex-1 overflow-y-auto min-h-0 pl-2 pr-[2px] gap-[1px] scrollbar-thin"}>
        {sel.displayNodes.length === 0 && (
          <div class="flex flex-col items-center justify-center flex-1 gap-3 px-4">
            ...
            <span
              class="text-muted-foreground text-[11px] leading-4 font-medium text-center"
              style={{ marginTop: "-4px" }}
            >
              {sel.searchQuery.length > 0
        ...
        {sel.hasMore && (
          <div class="flex items-center gap-1.5 h-[30px] pr-[4px] pl-[8px] rounded-sm border border-transparent shrink-0">
            {sel.isLoadingMore ? (
              <span class="text-[11px] leading-4 font-medium text-muted-foreground whitespace-nowrap">Loading...</span>
            ) : (
              <>
                <span
