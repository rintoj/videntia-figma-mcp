  ...
  if (editing) {
    return (
      <div class="flex flex-col gap-2 p-3 bg-muted border border-border rounded-[10px]">
        ...
        />
        <button
          class="w-full py-2 text-xs leading-4 bg-primary border border-solid border-primary text-primary-foreground rounded-md cursor-pointer font-medium transition-colors hover:brightness-110"
    ...
    return (
      ...
      >
        <div class="flex items-center gap-1.5 min-w-0 flex-1">
          ...
          <span
            class={
              "text-xs leading-4 whitespace-nowrap overflow-hidden text-ellipsis" +
              (readOnly ? " text-warning" : " text-primary")
            }
          >
  ...
  return (
    <div class="flex items-center justify-between px-3 py-1.5 gap-2.5">
      <div class="flex items-center gap-1.5 min-w-0 flex-1">
        ...
        {!connecting && (
          <span
            class="text-muted-foreground text-xs leading-4 cursor-pointer whitespace-nowrap hover:text-foreground"
      ...
      {connecting ? (
        <span class="flex items-center gap-1 text-muted-foreground text-xs leading-4 opacity-60">
        ...
        >
          <UnplugIcon color="currentColor" size={14} /> Retry
        </button>
      ) : (
        <button
        ...
        >
          <UnplugIcon color="currentColor" size={14} /> Connect
        </button>
      )}
    </div>
  );
