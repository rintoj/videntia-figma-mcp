...
function Tab({ id, label, active, icon, onTabChange }: TabProps) {
  return (
    <div
      class={
        "flex flex-row items-center justify-center gap-1 flex-1 py-2.5 cursor-pointer select-none transition-colors relative" +
        (active ? " bg-card tab-active-bar" : "")
      }
      onClick={function () {
        onTabChange(id);
      }}
    >
      ...
      <span class={"text-xs leading-4 font-medium" + (active ? " text-success" : " text-muted-foreground")}>
        {label}
      </span>
    </div>
  );
  ...
  return (
    <div class="flex items-center bg-popover border-b border-border shrink-0">
      <Tab
        ...
        icon={function (c) {
          return <ListIcon color={c} size={13} />;
        }}
      ...
      <Tab
        ...
        icon={function (c) {
          return <MousePointerIcon color={c} size={13} />;
        }}
        onTabChange={onTabChange}
      />
      <Tab
        ...
        icon={function (c) {
          return <GearIcon color={c} size={13} />;
        }}
        onTabChange={onTabChange}
      />
    </div>
