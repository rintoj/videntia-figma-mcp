...
export function ActionItem({ action }: ActionItemProps) {
  ...
  return (
    ...
    >
      <div
        class="flex items-center gap-1.5 cursor-pointer rounded-sm py-[7px] pl-2 pr-0.5 transition-colors hover:bg-accent"
        onClick={function () {
          setExpanded(!expanded);
        }}
      >
        <div class="flex items-center gap-1.5 min-w-0 flex-1 overflow-hidden">
          <span class="flex items-center shrink-0">{getStatusIcon()}</span>
          <span class="text-foreground text-xs leading-4 font-medium whitespace-nowrap overflow-hidden text-ellipsis">
            {action.command}
          </span>
        </div>
        <div class="flex items-center gap-2 shrink-0">
          ...
          <span class="text-muted-foreground font-mono text-[10px] leading-[14px] whitespace-nowrap">
      ...
      {expanded && (
        <div class="flex flex-col gap-1.5 pb-[7px] pl-2 pr-0.5">
          <div class="flex flex-row justify-end items-center gap-2.5 mt-1">
            {hasNodeIds && (
              <button
                class="flex items-center gap-1 py-[3px] px-[7px] bg-accent border border-transparent rounded-md text-muted-foreground text-[11px] leading-4 font-medium cursor-pointer transition-colors hover:bg-border hover:text-foreground"
                onClick={handleFocus}
              >
            ...
            {hasNodeIds && (
              <button
                class="flex items-center gap-1 py-[3px] px-[7px] bg-secondary border border-transparent rounded-md text-muted-foreground text-[11px] leading-4 font-medium cursor-pointer transition-colors hover:bg-border hover:text-foreground"
          ...
          <div class="bg-popover rounded-md py-2 px-2.5 relative overflow-hidden detail-block-accent flex flex-col gap-1">
            <span class="text-muted-foreground text-[11px] leading-4 font-medium uppercase tracking-[1.5px] block">
              INPUT
            </span>
            <pre class="text-muted-foreground font-mono text-[10px] leading-[14px] m-0 whitespace-pre-wrap break-all max-h-[120px] overflow-y-auto">
              {inputStr}
            </pre>
          </div>
          <div
            class={
              "bg-popover rounded-md py-2 px-2.5 relative overflow-hidden detail-block-accent flex flex-col gap-1" +
              (action.status === "success" ? " detail-block-success" : "") +
              (action.status === "error" ? " detail-block-error" : "")
            }
          >
            <span class="text-muted-foreground text-[11px] leading-4 font-medium uppercase tracking-[1.5px] block">
              OUTPUT
            ...
            <pre
              class={
                "font-mono text-[10px] leading-[14px] m-0 whitespace-pre-wrap break-all max-h-[120px] overflow-y-auto" +
