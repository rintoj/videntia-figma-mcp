...
function generateChannelName(fileName?: string | null): string {
  if (fileName) {
    ...
    var slug = fileName.toLowerCase().replace(/[^a-z0-9]+/g, "");
...
function formatCommandName(command: string): string {
  if (!command) return "";
  return command.replace(/_/g, " ").replace(/^\w/, function (c) {
...
export function useConnection() {
  ...
  function scheduleReconnect(wasConnected: boolean) {
    ...
    var delay = Math.min(RECONNECT_BASE_DELAY * Math.pow(2, reconnectAttemptRef.current), RECONNECT_MAX_DELAY);
    ...
    reconnectTimerRef.current = setTimeout(function () {
      ...
      setConnState(function (prev) {
        return Object.assign({}, prev, { buttonDisabled: true });
      });
  ...
  function handleCommandResult(message: any) {
    ...
    var resultDelay = resultStart !== undefined ? Math.max(0, MIN_PROGRESS_DISPLAY_MS - (Date.now() - resultStart)) : 0;
  ...
  function handleCommandError(message: any) {
    ...
    var errorDelay = errorStart !== undefined ? Math.max(0, MIN_PROGRESS_DISPLAY_MS - (Date.now() - errorStart)) : 0;
    progressStartTimesRef.current.delete(message.id);
    setTimeout(function () {
