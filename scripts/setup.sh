...
if [[ "$OSTYPE" == "darwin"* ]]; then
    if [ -n "$BUN_PATH" ]; then
        ...
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            PLIST_SRC="scripts/com.claude-figma-mcp.socket.plist"
            PLIST_DEST="$HOME/Library/LaunchAgents/com.claude-figma-mcp.socket.plist"
            PROJECT_PATH="$(pwd)"
            ...
            echo "  Logs: ~/Library/Logs/claude-figma-mcp-socket.log"
        fi
    else
        echo ""
