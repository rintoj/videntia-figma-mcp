...
if [[ "$OSTYPE" == "darwin"* ]]; then
    if [ -n "$BUN_PATH" ]; then
        ...
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            PLIST_SRC="scripts/videntia-figma-mcp.socket.plist"
            PLIST_DEST="$HOME/Library/LaunchAgents/videntia-figma-mcp.socket.plist"
            PROJECT_PATH="$(pwd)"
            ...
            echo "  Logs: ~/Library/Logs/videntia-figma-mcp-socket.log"
        fi
    else
        echo ""
...
echo "   Select: src/videntia_figma_plugin/manifest.json"
