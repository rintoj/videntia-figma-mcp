...
if [[ "$OSTYPE" == "darwin"* ]]; then
    if [ -n "$BUN_PATH" ]; then
        ...
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            PLIST_SRC="scripts/hgraph-figma-mcp.socket.plist"
            PLIST_DEST="$HOME/Library/LaunchAgents/hgraph-figma-mcp.socket.plist"
            PROJECT_PATH="$(pwd)"
            ...
            echo "  Logs: ~/Library/Logs/hgraph-figma-mcp-socket.log"
        fi
    else
        echo ""
...
echo "   Select: src/hgraph_figma_plugin/manifest.json"
