...
Add the corresponding MCP tool in `src/hgraph_figma_mcp/tools/*.ts` (this is the MCP server side, separate from the plugin).

### Step 4: Update `FigmaCommand` type

Add the new command string to the union in `src/hgraph_figma_mcp/types/index.ts`.
