...
The Figma plugin source (`src/videntia_figma_plugin/`) has been migrated from a single monolithic
...
```json
{
  ...
  "include": ["src/videntia_figma_plugin/**/*.ts"]
}
```
...
This outputs directly to `src/videntia_figma_plugin/code.js`, which is the file Figma loads via `manifest.json`. The TypeScript migration is complete and live.
...
```bash
grep -n '\?\.' src/videntia_figma_plugin/**/*.ts
grep -n ' \?\? ' src/videntia_figma_plugin/**/*.ts
```
...
Add the function to the appropriate handler in `src/videntia_figma_plugin/handlers/`.

```typescript
// src/videntia_figma_plugin/handlers/nodes.ts (example)
...
Add the corresponding MCP tool in `src/videntia_figma_mcp/tools/*.ts` (this is the MCP server side, separate from the plugin).

### Step 4: Update `FigmaCommand` type

Add the new command string to the union in `src/videntia_figma_mcp/types/index.ts`.
...
ls -lh src/videntia_figma_plugin/code.js  # verify output
```
...
ls -lh src/videntia_figma_plugin/code.js
```
...
The TypeScript migration is **complete**. The plugin builds directly to `src/videntia_figma_plugin/code.js`
...
bun run build                               # full build
ls -lh src/videntia_figma_plugin/code.js       # should be ~155 KB minified
```
